...
export default function DataTable() {
    ...
    const { data, isLoading, isError } = useGetNewsQuery({ page, limit });
    ...
    const handleDragEnd = async (result) => {
        ...
        setItems(updated)
        const patch = { order: result.destination.index + 1 };
        await dragAndDrop({ id: movedItem.id, patch });
    };
        ...
        <div>
            <div className="container p-2 mx-auto sm:p-4 dark:text-gray-800">
                <div className="flex items-center justify-between pb-4">
                    ...
                    <button
                        ...
                        className="bg-black cursor-pointer text-white px-2 py-3 rounded font-semibold"
                    >
                        Create News
                    </button>
                </div>
                {open && (
                    <ModalContain close={close}>
                        {newsImage && (
                            <div className="w-full h-auto relative">
                                ...
                                />
                                <Trash
                                    onClick={() => deletImage(newsImage?.id)}
                                    className="absolute top-[-10px] right-[-10px] w-8 h-8 bg-black p-1 rounded-full text-red-600"
                                />
                            </div>
                        )}
                        <h2>Image :</h2>
                        ...
                        <input
                            ...
                            required
                            placeholder="Actions"
                            className="mb-2 w-full border-b p-2 outline-0"
                        />
                        <h2>Description</h2>
                        <textarea
                            value={description}
                            ...
                            className="mb-2 w-full h-10 border-b p-2 outline-0"
                        ></textarea>
                        <button
                            disabled={newsLoader || mediaLoader || updateLoad}
                            onClick={createNews}
                            className="bg-black text-center w-full cursor-pointer text-white py-3 rounded font-semibold"
                        >
            ...
            {isLoading || deletLoader || deletMediaLoader || droploading ? (
                ...
                <DragDropContext onDragEnd={handleDragEnd}>
                    <Droppable droppableId="newsTable">
                        {(provided) => (
                            <table
            ...
            <div className='flex items-center justify-between'>
                ...
                <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
                    <InputLabel id="demo-simple-select-standard-label">Page Limit</InputLabel>
                    <Select
